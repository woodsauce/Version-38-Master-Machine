...
const APP_VERSION_FILE = 'edge15-genesis-master-machine-38_1-price-neutral-full-launch.zip';
const LEARNING_SAMPLE_TARGET = 25;
const STORE_KEY = 'edge15.master38.records.v1';
const SETTINGS_KEY = 'edge15.master38.settings.v1';
const LADDERS_KEY = 'edge15.master38.ladders.v1';
const CURRENT_LADDER_KEY = 'edge15.master38.currentLadder.v1';
const ODDS_CACHE_KEY = 'edge15.master38.oddsCache.v1';
const MAX_TICKS = 900;
...
const TRACKER_LOCK_VERSION = 38;
const CORE_MODEL_VERSION = 30;
const MASTER_MODEL_VERSION = 38;
...
const SIX_MINUTE_READ_EARLIEST_SEC = 390;
const SIX_MINUTE_READ_LATEST_SEC = 300;
const SECOND_CHANCE_EARLIEST_SEC = 300;
const SECOND_CHANCE_LATEST_SEC = 120;
const OVER_EARLY_MIN_CUSHION_SCORE = 90;
const OVER_EARLY_MIN_ENTRY_SCORE = 82;
...
const ELITE_MIN_STABILITY = 72;
const ELITE_MIN_ENTRY_QUALITY = 72;
const ELITE_MAX_FLIP_RISK = { OVER: 54, UNDER: 54 };
const ELITE_MIN_CHART_SCORE = { OVER: 64, UNDER: 64 };
const ELITE_MIN_SUCCESS_CHANCE = { OVER: 66, UNDER: 66 };
const ELITE_STRONG_CUSHION_SCORE = 115;
...
const GENESIS020_OVER_MIN_SCORE = 76;
const GENESIS020_UNDER_MAX_SCORE = 25;
...
const GENESIS_BALANCED_B_MIN_SUCCESS = 66;
...
function renderAppQr() {
  const img = $('appQr');
  const urlEl = $('appQrUrl');
  if (!img) return;
  const liveUrl = window.location.href;
  img.src = `https://api.qrserver.com/v1/create-qr-code/?size=180x180&margin=8&data=${encodeURIComponent(liveUrl)}`;
  if (urlEl) urlEl.textContent = liveUrl.replace(/^https?:\/\//, '');
}
...
function renderDecision(decision) {
  ...
  const officialSide = ['OVER', 'UNDER'].includes(officialLock?.action) ? officialLock.action : null;
  const displayAction = officialSide ? `LOCK ${officialSide}` : action;
  $('mainAction').textContent = displayAction;
  $('mainAction').className = `main-action ${(officialSide || action).toLowerCase()}${officialSide ? ' official-lock' : ''}`;
  ...
  renderMasterJudgePanel(decision);
...
function renderLockedSideBanner() {
  ...
  if (lock && ['OVER', 'UNDER'].includes(lock.action)) {
    const master = lock.masterJudgement || {};
    sideEl.textContent = `LOCK ${lock.action}`;
    detailEl.textContent = `OFFICIAL V38 MASTER LOCK · ${master.grade || 'LOCK'} · ${lock.minute}m · ${lock.source}`;
    banner.className = `locked-side-banner official ${String(lock.action).toLowerCase()}`;
    return;
  }
  ...
  detailEl.textContent = 'Master Lock: waiting for valid Core V30 + confirmation stack';
...
function updateOfficialLockAlertUi() {
  ...
  if (!card) return;
  const lock = state.currentLadder?.officialCall;
  const key = lock ? `${state.currentLadder?.windowKey}|${lock.lockedAt}|${lock.action}` : '';
  card.classList.toggle('officially-locked', Boolean(lock));
  if (banner) banner.classList.toggle('officially-locked', Boolean(lock));
  if (lock && key && state.lastOfficialLockFlashKey !== key) {
    state.lastOfficialLockFlashKey = key;
    card.classList.remove('tracker-locked-flash');
    if (banner) banner.classList.remove('tracker-locked-flash');
    // Force reflow so the quick flash animation restarts exactly once per official lock.
    void card.offsetWidth;
    card.classList.add('tracker-locked-flash');
    if (banner) banner.classList.add('tracker-locked-flash');
    setTimeout(() => {
      card.classList.remove('tracker-locked-flash');
      if (banner) banner.classList.remove('tracker-locked-flash');
    }, 1800);
...
function getEliteAccuracyGuard(decision, candidate = {}, source = '') {
  ...
  const isEmergency = normalizedSource.includes('genesis emergency money lock') || normalizedSource.includes('genesis emergency confirmed lock');
...
function getSixMinuteReadGuard(decision, candidate = {}) {
  ...
  // V30: this is intentionally a 6-minute read lock, not a loose early scout.
  ...
  if (confidence < 78) blockers.push(`6m read confidence ${confidence.toFixed(1)} below 78.`);
  if (stability < 50) blockers.push(`6m read stability ${stability.toFixed(1)} below 50.`);
  if (flipRisk > 68) blockers.push(`6m read flip risk ${flipRisk.toFixed(1)} above 68.`);
  if (entryQuality < 58) blockers.push(`6m read entry quality ${entryQuality.toFixed(1)} below 58.`);
  if (!chart.available || !chart.supports || chart.conflict) blockers.push('6m read requires chart support without conflict.');
  if (chartScore < 58) blockers.push(`6m read chart score ${chartScore.toFixed(1)} below 58.`);
  if (cushionScore < 115) blockers.push(`6m read requires strong cushion; score ${cushionScore.toFixed(0)} below 115.`);
  ...
  if (grade === 'B' && String(genesis.settlementRisk || '') !== 'Low') blockers.push('6m B-grade read requires Low settlement risk.');
  if (successChance < 62) blockers.push(`6m read Genesis success ${successChance.toFixed(1)} below 62.`);
  if (side === 'OVER' && Number.isFinite(entryScore) && entryScore < 70) blockers.push(`6m OVER entryScore ${entryScore.toFixed(0)} below 70.`);
  if (side === 'UNDER' && Number.isFinite(entryScore) && entryScore > 32) blockers.push(`6m UNDER entryScore ${entryScore.toFixed(0)} above 32.`);
  if (vwapSide && vwapSide !== side) blockers.push(`6m read VWAP is against ${side}.`);
  if (targetCrosses > 18) blockers.push(`6m read target crosses ${targetCrosses} above 18.`);
  else if (targetCrosses > 6) warnings.push(`Target crossed ${targetCrosses} times; accepted only because cushion is huge.`);
  if (cushionVelocity < -20) blockers.push(`6m cushion velocity ${cushionVelocity.toFixed(1)} is deteriorating too fast.`);
  if (value.available === false) warnings.push(`Odds unavailable; decision still allowed because odds/profit are display-only: ${value.reason || value.status || 'unavailable'}.`);
  if (Number.isFinite(oddsAgeSec) && oddsAgeSec > ELITE_MAX_ODDS_AGE_SEC) warnings.push(`Odds age ${oddsAgeSec}s above ${ELITE_MAX_ODDS_AGE_SEC}s; odds freshness is display-only, not a prediction blocker.`);
  ...
  return { pass: blockers.length === 0, blockers, warnings, chartScore, successChance, stability, flipRisk, entryQuality, cushionScore, liveBalancedV30: true };
}


function sameSideSign(side, value, deadZone = 0) {
  const n = Number(value || 0);
  if (side === 'OVER') return n > deadZone;
  if (side === 'UNDER') return n < -deadZone;
  return false;
}

function oppositeSideSign(side, value, deadZone = 0) {
  const n = Number(value || 0);
  if (side === 'OVER') return n < -deadZone;
  if (side === 'UNDER') return n > deadZone;
  return false;
...
function classifyMasterRegime(side, chart = {}) {
  const targetCrosses = Number(chart.targetCrosses || 0);
  const momentum60 = Number(chart.momentum60s || 0);
  const momentum3m = Number(chart.momentum3m || 0);
  const rsi = Number(chart.rsi);
  const chartScore = side === 'UNDER' ? Number(chart.underScore || chart.score || 0) : Number(chart.overScore || chart.score || 0);
  const vwapSide = String(chart.vwapSide || '').toUpperCase();
  const emaTrend = String(chart.emaTrend || '').toUpperCase();
  const alignedMomentum = sameSideSign(side, momentum60, 2) && sameSideSign(side, momentum3m, 2);
  const opposingMomentum = oppositeSideSign(side, momentum60, 3) || oppositeSideSign(side, momentum3m, 3);
  if (targetCrosses >= 8) return { label: 'chop', quality: 18, confirm: false, veto: true, reason: `target crossed ${targetCrosses} times` };
  if (side === 'OVER' && Number.isFinite(rsi) && rsi >= 78 && momentum60 > 8 && momentum3m < momentum60 * 0.45) return { label: 'over-exhaustion', quality: 24, confirm: false, veto: true, reason: 'OVER momentum is fading after a hot RSI push' };
  if (side === 'UNDER' && Number.isFinite(rsi) && rsi <= 18 && momentum60 < -8 && momentum3m > momentum60 * 0.45) return { label: 'under-exhaustion', quality: 24, confirm: false, veto: true, reason: 'UNDER momentum is fading after oversold pressure' };
  if (alignedMomentum && chartScore >= 72 && targetCrosses <= 4 && vwapSide === side) return { label: 'trend-continuation', quality: 92, confirm: true, veto: false, reason: 'momentum, VWAP, and target behavior align' };
  if (alignedMomentum && targetCrosses <= 4 && emaTrend !== (side === 'OVER' ? 'UNDER' : 'OVER')) return { label: 'directional-lean', quality: 78, confirm: true, veto: false, reason: 'directional lean without heavy chop' };
  if (opposingMomentum) return { label: 'counter-move', quality: 35, confirm: false, veto: false, reason: 'momentum is not fully aligned' };
  return { label: 'mixed', quality: 55, confirm: false, veto: false, reason: 'mixed regime' };
}

function getMasterSensorStack(decision, candidate = {}, remainingSec = getRemainingSec()) {
  const side = String(decision?.action || '').toUpperCase();
  const chart = candidate.chartSupport || getChartSignalSupport(decision);
  const genesis = candidate.genesisGate || getGenesisGateSnapshot(decision, remainingSec);
  const cushion = candidate.cushionSnapshot || {};
  const value = candidate.valueSnapshot || getExpectedValueSnapshot(decision);
  const source = String(candidate.source || '').toLowerCase();
  const confirmations = [];
  const warnings = [];
  const vetoes = [];
  const sensors = [];
  const addSensor = (name, status, note, weight = 0) => {
    sensors.push({ name, status, note, weight });
    if (status === 'confirm') confirmations.push(name);
    if (status === 'veto') vetoes.push(`${name}: ${note}`);
    if (status === 'warn') warnings.push(`${name}: ${note}`);
  };

  if (!['OVER', 'UNDER'].includes(side)) {
    return { pass: false, score: 0, grade: 'SKIP', confirmations, warnings, vetoes: ['No OVER/UNDER candidate'], sensors, overGate: 'n/a' };
  }

  const chartScore = side === 'UNDER' ? Number(chart.underScore || chart.score || 0) : Number(chart.overScore || chart.score || 0);
  const otherScore = side === 'UNDER' ? Number(chart.overScore || 0) : Number(chart.underScore || 0);
  const targetCrosses = Number(chart.targetCrosses || 0);
  const vwapSide = String(chart.vwapSide || '').toUpperCase();
  const emaTrend = String(chart.emaTrend || '').toUpperCase();
  const momentum60 = Number(chart.momentum60s || 0);
  ...
  const successChance = Number(genesis.successChance || 0);
  const entryScore = Number(genesis.entryScore);
  const totalRisk = Number(genesis.totalRisk || 100);
  const flipRisk = Number(decision.flipRisk || 100);
  const stability = Number(decision.stability || 0);
  const entryQuality = Number(candidate.entryQuality || getTimingSignal(decision).entryQuality || 0);
  const cushionVelocity = side === 'UNDER' ? Number(chart.underCushionVelocity || 0) : Number(chart.overCushionVelocity || 0);
  const oddsPercent = Number(value.oddsPercent || 0);
  const oddsFresh = value.available && Number(value.oddsAgeSec ?? 999) <= ELITE_MAX_ODDS_AGE_SEC;
  const regime = classifyMasterRegime(side, chart);
  const sideOpposite = side === 'OVER' ? 'UNDER' : 'OVER';

  let score = 42;
  if (candidate.allow) { score += 18; addSensor('Core V30', 'confirm', 'base candidate passed V30 guardrails', 18); }
  else addSensor('Core V30', 'warn', 'base candidate is not fully approved yet', 0);

  const v33Confirm = chart.available && chartScore >= (side === 'OVER' ? 72 : 68) && vwapSide === side && sameSideSign(side, momentum60, 2) && !chart.conflict;
  if (v33Confirm) { score += 12; addSensor('V33 Pro Trader', 'confirm', `technical stack supports ${side}`, 12); }
  else if (chart.available) { score -= side === 'OVER' ? 5 : 2; addSensor('V33 Pro Trader', 'warn', `technical stack not clean enough (${chartScore.toFixed(1)})`, -5); }

  if (regime.veto) { score -= 22; addSensor('V32 Regime', 'veto', regime.reason, -22); }
  else if (regime.confirm) { score += 10; addSensor('V32 Regime', 'confirm', regime.reason, 10); }
  else { score += Math.max(-4, Math.min(4, (regime.quality - 55) / 6)); addSensor('V32 Regime', 'warn', regime.reason, 0); }

  const trendConfirm = targetCrosses <= 4 && sameSideSign(side, momentum60, 1) && sameSideSign(side, momentum3m, 1) && emaTrend !== sideOpposite;
  if (trendConfirm) { score += 10; addSensor('Trend Follow', 'confirm', 'trend and cushion direction agree', 10); }
  else if (oppositeSideSign(side, momentum60, 1) || emaTrend === sideOpposite) { score -= 10; addSensor('Trend Follow', 'veto', 'trend is fighting the proposed side', -10); }
  else addSensor('Trend Follow', 'warn', 'trend is mixed', 0);

  // V38.1: odds/price/EV/profit are fully display-only.
  // They do not add confirmations, remove confirmations, affect score, or block a valid prediction lock.
  // A fixed neutral +4 preserves the prior score baseline without making cheap odds better than expensive odds.
  score += 4;
  if (value.available === false) addSensor('Odds / Price', 'info', 'ignored for lock decision; unavailable is OK', 0);
  else addSensor('Odds / Price', 'info', `${oddsPercent.toFixed(0)}¢ shown only; never blocks or helps`, 0);

  const snapbackVeto = (side === 'OVER' && ((targetCrosses >= 8) || (emaTrend === 'UNDER' && chartScore < 75) || (Number.isFinite(rsi) && rsi >= 82 && (rsiSlope < -2 || momentum15 < 0)))) ||
    (side === 'UNDER' && ((targetCrosses >= 10) || (emaTrend === 'OVER' && chartScore < 72) || (Number.isFinite(rsi) && rsi <= 16 && (rsiSlope > 2 || momentum15 > 0))));
  if (snapbackVeto) { score -= 28; addSensor('Snapback Guard', 'veto', 'reversal/chop danger against the lock', -28); }
  else { score += 8; addSensor('Snapback Guard', 'confirm', 'no hard snapback veto', 8); }

  if (cushionScore >= 175) score += 6;
  else if (cushionScore >= 115) score += 3;
  else score -= 8;
  if (successChance >= 74) score += 5;
  else if (successChance < 66) score -= 10;
  if (stability >= 90) score += 4;
  if (entryQuality >= 82) score += 4;
  if (flipRisk > 50) score -= 6;
  if (totalRisk > 42) score -= 6;
  if (targetCrosses > 4) score -= Math.min(20, (targetCrosses - 4) * 4);
  if (cushionVelocity < -4) score -= 8;
  if (otherScore > chartScore - 10) score -= 5;
  ...
  if (side === 'OVER') {
    const overPasses = [
      ...
      emaTrend !== 'UNDER',
      Number(momentum60) > 0,
      Number(momentum3m) > 0 && Number(momentum3m) >= Math.min(5, Number(momentum60) * 0.15),
      !Number.isFinite(rsi) || rsi < 78,
      targetCrosses <= 4,
      v33Confirm,
      trendConfirm,
      !regime.veto,
    ...
    overGate = `${overPasses}/10`;
    if (targetCrosses >= 8) vetoes.push(`OVER Protection Gate: target crossed ${targetCrosses} times`);
    if (vwapSide === 'UNDER') vetoes.push('OVER Protection Gate: VWAP is under target side');
    if (emaTrend === 'UNDER' && chartScore < 76) vetoes.push(`OVER Protection Gate: EMA down and chart score ${chartScore.toFixed(1)}`);
    if (chartScore < 66) vetoes.push(`OVER Protection Gate: chart score ${chartScore.toFixed(1)} below 66`);
    else if (source.includes('read') && chartScore < 72) vetoes.push(`OVER Protection Gate: 6m read chart score ${chartScore.toFixed(1)} below 72`);
    if (momentum60 <= 0) vetoes.push('OVER Protection Gate: 60s momentum is not positive');
    if (momentum3m <= 0 && source.includes('read')) vetoes.push('OVER Protection Gate: 6m read needs positive 3m momentum');
    if (Number.isFinite(entryScore) && entryScore < 78) vetoes.push(`OVER Protection Gate: Genesis entry score ${entryScore.toFixed(0)} below 78`);
    if (source.includes('6m genesis balanced read') && overPasses < 7) vetoes.push(`OVER Protection Gate: 6m read has only ${overPasses}/10 confirmations`);
    if (overPasses < 6) score -= 20;
    else score += Math.min(8, overPasses - 5);
  }

  const hardVetoes = vetoes.filter(Boolean);
  const lockThreshold = side === 'OVER' ? 82 : 75;
  const grade = score >= 90 ? 'A-LOCK' : score >= lockThreshold ? 'B-LOCK' : score >= 60 ? 'WAIT' : 'SKIP';
  const pass = hardVetoes.length === 0 && score >= lockThreshold && confirmations.length >= (side === 'OVER' ? 4 : 2);
  ...
  return {
    ...
    summary: pass ? `${grade}: ${confirmations.slice(0, 4).join(' + ')}` : `${grade}: ${hardVetoes[0] || warnings[0] || 'waiting for more confirmation'}`
  };
}

function applyMasterMachineGuard(decision, candidate = {}, remainingSec = getRemainingSec()) {
  const master = getMasterSensorStack(decision, candidate, remainingSec);
  if (!master.pass) {
    return {
      ...candidate,
      allow: false,
      masterJudgement: master,
      reason: `V38 Master blocked: ${master.vetoes?.slice(0, 2).join(' · ') || master.summary}`
    };
  }
  return {
    ...candidate,
    masterJudgement: master,
    reason: `V38 ${master.summary}`
  };
}

function renderMasterJudgePanel(decision = state.decision) {
  const el = $('masterJudgePanel');
  if (!el) return;
  const lock = state.currentLadder?.officialCall;
  let master = lock?.masterJudgement || null;
  let candidate = null;
  if (!master && decision && ['OVER', 'UNDER'].includes(decision.action)) {
    candidate = getOfficialLockCandidate(decision, getRemainingSec());
    master = candidate.masterJudgement || null;
  }
  if (!master) {
    el.innerHTML = '<div class="master-row"><strong>Master Judge</strong><span>Waiting for Core V30 candidate</span></div>';
    return;
  }
  const sensorRows = (master.sensors || []).slice(0, 6).map((sensor) => `<div class="sensor ${escapeHtml(sensor.status)}"><strong>${escapeHtml(sensor.name)}</strong><span>${escapeHtml(sensor.status.toUpperCase())}</span><em>${escapeHtml(sensor.note || '')}</em></div>`).join('');
  const vetoText = master.vetoes?.length ? master.vetoes.slice(0, 2).join(' · ') : 'No hard veto';
  el.innerHTML = `
    <div class="master-head ${master.pass ? 'good' : master.grade === 'WAIT' ? 'warn' : 'bad'}">
      <span>Master Judge</span><strong>${escapeHtml(master.grade || 'WAIT')} · ${master.score || 0}%</strong>
    </div>
    <div class="master-row"><strong>Confirmations</strong><span>${escapeHtml(String(master.confirmations?.length || 0))}</span><em>${escapeHtml((master.confirmations || []).slice(0, 4).join(' + ') || 'None yet')}</em></div>
    <div class="master-row"><strong>Veto</strong><span>${escapeHtml(vetoText)}</span></div>
    <div class="master-row"><strong>OVER gate</strong><span>${escapeHtml(String(master.overGate || 'n/a'))}</span><em>Only applies to OVER protection</em></div>
...
function getOfficialLockCandidate(decision, remainingSec) {
  ...
  const bGradeClean = grade !== 'B' || (genesisGate.settlementRisk === 'Low' && strongCushion && sideChartScore >= 74);
  const commonStrict = baseClean && entryQuality >= ELITE_MIN_ENTRY_QUALITY && rightSideOnly && strongCushion && chartClean && genesisClean && gradeClean && bGradeClean && genesis020.pass && cushionVelocity >= -2;
  ...
  const secondChanceLock = remainingSec < SECOND_CHANCE_EARLIEST_SEC && remainingSec >= SECOND_CHANCE_LATEST_SEC && commonStrict && overStrict && underStrict && confidence >= 86 && stability >= 88 && flipRisk <= 40 && entryQuality >= 80;
  ...
  const approve = (source, entryEligible = remainingSec >= GENESIS_MONEY_LATEST_SEC) => {
    ...
    const masterGuard = applyMasterMachineGuard(decision, { ...candidate, eliteGuard }, remainingSec);
    if (!masterGuard.allow) return { ...masterGuard, eliteGuard };
    return { ...masterGuard, eliteGuard, reason: masterGuard.reason || `${source}: V38 Master confirmation passed.` };
  };
  ...
  const approveSixMinuteRead = () => {
    ...
    const masterGuard = applyMasterMachineGuard(decision, { ...candidate, sixMinuteGuard, eliteGuard: sixMinuteGuard }, remainingSec);
    if (!masterGuard.allow) return { ...masterGuard, sixMinuteGuard, eliteGuard: sixMinuteGuard };
    return {
      ...masterGuard,
      sixMinuteGuard,
      eliteGuard: sixMinuteGuard,
      reason: masterGuard.reason || '6m genesis balanced read lock: V38 Master confirmation passed.'
    };
  };

  if (ultraEarlyEmergency) return approve('Genesis-020 ultra emergency lock');
  if (emergency) return approve('genesis emergency confirmed lock');
  if (earlyGenesisLock) {
    return {
      allow: false,
      reason: '8m Genesis early lock is preview-only in V30; official money entry starts at the 6m read / 4m-2m second-chance windows.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
  ...
  if (sixMinuteMoneyLock) return approve('6m genesis balanced confirmed lock');
  ...
  // V30: if the strict second-chance path does not fire, reuse the balanced live-read guard late.
  ...
  if (remainingSec < SECOND_CHANCE_EARLIEST_SEC && remainingSec >= SECOND_CHANCE_LATEST_SEC) {
    ...
    if (lateCandidate.allow) return { ...lateCandidate, source: '4m-2m live balanced catch lock', reason: '4m-2m live balanced catch lock: late signal cleaned up under V30 guardrails.' };
  }
  if (tenMinutePreviewOnly) {
    return {
      allow: false,
      reason: '10m Genesis elite is preview-only; V30 waits for the 6m read or late second-chance lock.',
      minute,
      entryQuality,
      heldMs,
      valueSnapshot,
...
function maybeLockOfficialCall(decision, remainingSec) {
  ...
  ladder.officialCall = {
    ...
    masterJudgement: candidate.masterJudgement || null,
    masterModelVersion: MASTER_MODEL_VERSION
  };
  saveCurrentLadder();
  renderLiveReturnTracker();
...
function getExpectedValueSnapshot(decision = state.decision) {
  ...
  const priceWarning = odds.oddsPercent >= TOO_EXPENSIVE_ODDS_PERCENT || (odds.oddsPercent >= EXPENSIVE_ODDS_PERCENT && evPercent < 6) || (winProfit < 0.07 && winProbability < 0.94);
  const valueLabel = evPercent >= 8 ? 'good value' : evPercent >= MIN_EV_PERCENT ? 'thin payout' : 'poor payout';
...
function getMoneyValueGate(decision = state.decision) {
  ...
  if (snapshot.priceWarning || snapshot.evPercent < MIN_EV_PERCENT) {
    return { allow: true, warningOnly: true, reason: `Display only: ${snapshot.status.toLowerCase()} at ${snapshot.oddsPercent}% odds. Tracker locks are never blocked by payout, EV, or price.`, snapshot };
  }
  return { allow: true, warningOnly: false, reason: 'Payout display only', snapshot };
}
